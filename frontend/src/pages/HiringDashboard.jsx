  ...
  if (loading) {
    ...
    <motion.div style={styles.page} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }}>
        <div style={styles.container}>
          ...
          <SkeletonCardGrid count={3} style={{ marginBottom: 24 }} />
      ...
      <div style={styles.container}>
        ...
        <div style={styles.topBar}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12 }}>
            <button style={styles.goldOutlineBtn} onClick={() => navigate('/dashboard')}>← Main Dashboard</button>
          ...
          <div style={styles.statsGrid}>
            ...
            <div style={{ ...styles.statCard, display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
              ...
              <div style={{ display: 'flex', alignItems: 'flex-end', gap: 10, marginTop: 8 }}>
                {[
                  { label: 'Sessions', value: stats.totalSessions ?? 0, color: '#c4a052' },
                  { label: 'Evaluated', value: stats.evaluatedCandidates ?? 0, color: '#2f8a5a' },
                  { label: 'Candidates', value: stats.totalCandidates ?? 0, color: '#5b9bd5' },
                ].map((item) => (
                  <div key={item.label} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', flex: 1, gap: 4 }}>
                    <span style={{ color: item.color, fontWeight: 800, fontSize: 18 }}>{item.value}</span>
                    <div style={{ width: '100%', height: Math.max(6, Math.min(36, item.value * 4)), background: item.color, borderRadius: 4, transition: 'height 0.4s ease' }} />
                    <span style={{ color: '#86868b', fontSize: 9, textTransform: 'uppercase', letterSpacing: 0.5 }}>{item.label}</span>
                  </div>
                ))}
              </div>
            </div>
