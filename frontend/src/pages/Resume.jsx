...
export default function Resume() {
  ...
  const handleExportPDF = () => {
    try {
      ...
      doc.text('HireCraft Resume Analysis', pw / 2, y, { align: 'center' });
      y += 8;
      goldRule();
      ...
      if (chatMessages.length > 0) {
        ...
        for (const m of chatMessages) {
          ...
          ensureSpace(12);
          writeLines(isUser ? 'You' : 'HireCraft Advisor', mx, 9, isUser ? [139, 92, 246] : [160, 160, 165], 'bold');
      ...
      doc.text('Generated by HireCraft — AI Resume Analysis Platform', pw / 2, y, { align: 'center' });

      doc.save(`HireCraft_Resume_Analysis_${new Date().toISOString().slice(0, 10)}.pdf`);
    } catch (err) {
  ...
  const handleExportHistoryPDF = async (e, id) => {
    ...
    try {
      ...
      drawBg(); y = mx + 6;
      doc.setFontSize(20); doc.setFont('helvetica', 'bold'); doc.setTextColor(232, 232, 235);
      doc.text('HireCraft Resume Analysis', pw / 2, y, { align: 'center' }); y += 8; rule();
      doc.setFontSize(10); doc.setFont('helvetica', 'normal'); doc.setTextColor(107, 107, 112);
      ...
      if (data.chatMessages?.length) {
        ...
        for (const m of data.chatMessages) {
          ...
          writeLines(isU ? 'You' : 'HireCraft Advisor', mx, 9, isU ? [139, 92, 246] : [160, 160, 165], 'bold');
          writeLines((m.content || '').replace(/\*\*/g, ''), mx + 2, 10, [200, 200, 204]); y += 3;
        }
      }
      doc.setFontSize(8); doc.setFont('helvetica', 'normal'); doc.setTextColor(107, 107, 112);
      y += 4; doc.text('Generated by HireCraft — AI Resume Analysis Platform', pw / 2, y, { align: 'center' });
      doc.save(`HireCraft_Resume_${new Date(data.createdAt).toISOString().slice(0, 10)}.pdf`);
    } catch (_) {}
  };
