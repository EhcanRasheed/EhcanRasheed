...
export default function Chatbot() {
  ...
  const [messages, setMessages] = useState([
    {
      role: 'bot',
      content: 'I am your Hire-Craft Interview Mentor. Ask me any question related to your interview.',
      time: new Date(),
    },
  ]);
  ...
  const handleExportHistoryPDF = async (e, id) => {
    ...
    try {
      ...
      doc.text('Hire-Craft Interview Transcript', pw / 2, y, { align: 'center' }); y += 8; goldRule();
      ...
      for (const m of msgs) {
        ...
        ensureSpace(12);
        writeLines(isUser ? 'You' : 'Hire-Craft Mentor', mx, 9, isUser ? [196, 160, 82] : [160, 160, 165], 'bold');
      ...
      doc.text('Generated by Hire-Craft — AI Interview Preparation Platform', pw / 2, y, { align: 'center' });
      doc.save(`Hire-Craft_Transcript_${new Date(session.createdAt).toISOString().slice(0, 10)}.pdf`);
    } catch (_) {}
  };
  ...
  const handleNewInterview = () => {
    window.speechSynthesis.cancel();
    setMessages([{ role: 'bot', content: 'I am your Hire-Craft Interview Mentor. Ask me any question related to your interview.', time: new Date() }]);
    setInput('');
  ...
  const handleExportPDF = () => {
    ...
    /* title */
    doc.setFontSize(20); doc.setFont('helvetica', 'bold'); doc.setTextColor(232, 232, 235);
    doc.text('Hire-Craft Interview Transcript', pw / 2, y, { align: 'center' });
    y += 8;
    goldRule();
    ...
    for (const m of messages) {
      const isUser = m.role === 'user';
      const who = isUser ? 'You' : 'Hire-Craft Mentor';
    ...
    doc.save(`Hire-Craft_Transcript_${sessionStart.toISOString().slice(0, 10)}.pdf`);
    } catch (err) {
    ...
    <AppLayout activePage="chatbot">
      <header style={styles.topBar}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '12px' }}>
          <div>
            <h1 style={styles.pageTitle}>Hire-Craft Interview Mentor</h1>
